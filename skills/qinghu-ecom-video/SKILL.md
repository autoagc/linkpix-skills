...
qhkit video generate '{"modelLabel":"全能电商2.0","duration":15,"prompt":"户外工作灯广告","uploadedImages":["./商品图.jpg"]}'
...
|---|---|---|---|---|---|
| `全能电商2.0` | 5–15 秒（15） | 带货视频·默认首选 | 效果好、提示词要求低 | ✅ 最多3 | 2 |
| `Seedance2.0` | 5–15 秒（15） | 带货视频·效果最好 | 智能规划镜头组 | ✅ 最多3 | 3.3 |
| `Seedance2.5` | 5–30 秒（30） | 营销广告·长视频 | 参考图必填 | ✅ 最多10 | 3.6 |
| `阿里wanx3.0` | 5–30 秒（15） | 营销素材 | 提示词要精准；参考视频总时长 + 生成时长 ≤30 秒 | ✅ 最多5 | 1.8 |
| `MiniMax H3` | 5–15 秒（10） | 性价比 | 768p、有超宽屏 21:9 | ❌ | 1 |
| `Happy Horse 1.1` | 5–15 秒（15） | 营销素材 | 音视频一体、画面稳定 | ❌ | 2 |
| `可灵3.0 Omni` | 5–15 秒（10） | 营销素材 | 角色统一、原生音画 | ✅ 最多1 | 4 |
| `Viduq3-turbo`（原品牌质感大片） | 5–16 秒（15） | 主图视频·电影级 | 参考图必填、提示词要精准 | ❌ | 0.7 |
| `Viduq2-turbo`（原品牌质感大片 5秒） | 仅 5 秒 | 主图视频·试水 | 有免费额度、参考图必填（首帧+尾帧）、只能 1 条 | ❌ | 0.4 |
| `电商热卖引擎` | 10 / 15 秒 | 爆款直出 | 营销逻辑重构镜头 | ❌ | 2 |

- 用户说了时长就按用户的（超出该模型范围 CLI 会报错并给出可选范围）；没说就提议默认时长并在确认参数时告诉用户可以改，提交时一定显式传 `duration`——时长越长越贵。
- 「提示词要求精准」的模型（Viduq3-turbo、阿里wanx3.0）给一句话会出废片——先帮用户把镜头、光影、材质补充完整再提交。
